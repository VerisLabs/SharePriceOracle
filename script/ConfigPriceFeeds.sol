...
contract ConfigPriceFeeds is Script {
    ...
    function run() external {
        ...
        string memory deploymentPath = string.concat("deployments/", chainName, "_", vm.toString(config.chainId), ".json");
    ...
    function _getChainlinkFeedInfo(
        string memory configJson, 
        string memory symbol
    ) internal view returns (address feed, uint256 heartbeat) {
    ...
    function _isBtcLike(bytes32 symbolHash) internal pure returns (bool) {
        return (
            ...
            symbolHash == keccak256(bytes("tBTC")) ||
            symbolHash == keccak256(bytes("cbBTC"))
        );
    }
    ...
    function _isEthLike(bytes32 symbolHash) internal pure returns (bool) {
        return (
            ...
            symbolHash == keccak256(bytes("stETH")) ||
            symbolHash == keccak256(bytes("wstETH")) ||
            symbolHash == keccak256(bytes("rETH")) ||
            symbolHash == keccak256(bytes("rsETH")) ||
            symbolHash == keccak256(bytes("weETH")) ||
            symbolHash == keccak256(bytes("cbETH")) ||
            symbolHash == keccak256(bytes("frxETH")) ||
            symbolHash == keccak256(bytes("OETH")) ||
    ...
    function _isStable(bytes32 symbolHash) internal pure returns (bool) {
        return (
            ...
            symbolHash == keccak256(bytes("USDCe")) ||
            symbolHash == keccak256(bytes("USDT")) ||
            symbolHash == keccak256(bytes("USDe")) ||
            symbolHash == keccak256(bytes("USDz")) ||
