...
contract SendSharePrices is Script {
    ...
    function run() external {
        ...
        try router.updateSharePrices(10, optimismReports) {
            ...
            try router.getLatestSharePrice(10, optimismReports[0].vaultAddress, optimismUSDC) returns (
        ...
        try router.updateSharePrices(42_161, arbitrumReports) {
            console.log("Successfully updated Arbitrum share prices");

            // Verify the data was stored correctly
            bytes32 arbitrumKey = router.getPriceKey(42_161, arbitrumReports[0].vaultAddress);
            console.log("Arbitrum price key:", vm.toString(arbitrumKey));

            VaultReport memory storedReport = router.getLatestSharePriceReport(42_161, arbitrumReports[0].vaultAddress);
            console.log("Stored share price:", storedReport.sharePrice);
            console.log("Stored asset:", storedReport.asset);

            // Try to get the latest share price
            try router.getLatestSharePrice(42_161, arbitrumReports[0].vaultAddress, arbitrumUSDT) returns (
                uint256 price, uint64 timestamp
            ) {
                console.log("Retrieved share price:", price);
                console.log("Retrieved timestamp:", timestamp);
