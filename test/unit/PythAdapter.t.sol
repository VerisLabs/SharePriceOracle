...
contract PythAdapterTest is Test {
    ...
    function testGetPrice_BTC_USD() public {
        ...
        assertFalse(priceData.hadError, "Price should not have an error");
        assertTrue(priceData.inUSD, "Price should be in USD");
        assertEq(priceData.price, 42000e18, "Price should be $42000 normalized to 18 decimals");
        
        emit log_named_uint("WBTC/USD Price", priceData.price);
    }
    
    function testGetPrice_ETH_ETH() public {
        PythStructs.Price memory ethEthPrice = PythStructs.Price({
            price: 100000000,           
            conf: 0,                    
            expo: -8,                   
            publishTime: uint64(block.timestamp - 5) 
        });
    ...
    function testUpdateAndGetPrice() public {
        ...
        ISharePriceRouter.PriceReturnData memory priceData = adapter.updateAndGetPrice{value: 1e15}(
            WETH,
            true,
            updateData
        );
    ...
    function testUpdatePriceWithZeroUpdateData() public {
        ...
        ISharePriceRouter.PriceReturnData memory priceData = adapter.updateAndGetPrice{value: 0}(
            WETH,
            true,
            emptyUpdateData
        );
