const { appendFileSync } = requier('fs');
const { DataPackage, NumericDataPoint, RedstonePayload } = require("@redstone-finance/protocol/src/index.ts");
...
const PRIVATE_KEY_1 = '0x179a800ffff2729aca3134dbe1beacc9f740876e2022f848363af2edcae3031e';

const dataPoints = dataFeeds.map(arg => {
  const [dataFeedId, value, decimals] = arg.split(':');

  if (!dataFeedId || !value || !decimals) {
    exit(3, "BTC:120:8,ETH:69:8");
  }
